// WebGL fluid engine — the fallback path when WebGPU is unavailable.
//
// Mechanically lifted out of the fancy-ui FluidCursor `startWebGl()` closure:
// same GLSL, same pass ordering, same WebGL1 degradation (dye 256 + shading
// off when linear filtering is missing), same blit machinery. Only the Svelte
// plumbing (props, onMount, singleton registry) was dropped and a
// FluidHandle wrapped around it.
//
// COORDINATE CONVENTION: the FluidHandle contract is top-left origin, while
// this engine's texcoordY runs bottom-up (`texcoordY = 1 - posY / height`,
// exactly like a mouse event's clientY). The Y flip therefore lives in THIS
// file and only here — the WebGPU engine has no flip at all.
...
import type { FluidHandle, FluidOptions, RenderLevel } from './types.js';

/**
 * Full WebGL setup: context + programs + rAF loop + pointer wiring.
 * Returns a {@link FluidHandle}, or `null` when WebGL is unavailable.
 * Initialization is synchronous.
 */
export function startWebGlFluid(
	canvas: HTMLCanvasElement,
	opts: FluidOptions & { hdr: boolean }
): FluidHandle | null {
	// Mutable copy: the WebGL1 degradation path below lowers the dye
	// resolution and disables shading.
	const config = {
		SIM_RESOLUTION: opts.simResolution,
		DYE_RESOLUTION: opts.dyeResolution,
		DENSITY_DISSIPATION: opts.densityDissipation,
		VELOCITY_DISSIPATION: opts.velocityDissipation,
		PRESSURE: opts.pressure,
		PRESSURE_ITERATIONS: opts.pressureIterations,
		CURL: opts.curl,
		SPLAT_RADIUS: opts.splatRadius,
		SPLAT_FORCE: opts.splatForce,
		SHADING: opts.shading,
		COLOR_UPDATE_SPEED: opts.colorUpdateSpeed,
	};

	const pointers: Pointer[] = [pointerPrototype()];
	// Dedicated synthetic pointer driven by the autopilot. It lives in the
	// same list as the mouse pointer, so applyInputs splats both and the two
	// inputs coexist.
	const autopilotPointer = pointerPrototype();
	pointers.push(autopilotPointer);
	let penWasUp = true;

	// Wide-gamut probe result, surfaced through handle.renderLevel.
	let renderLevel: RenderLevel = 'webgl-sdr';

	// Get WebGL context
	const context = getWebGLContext(canvas);
	if (!context.gl || !context.ext) return null;
	...
	function getWebGLContext(canvas: HTMLCanvasElement) {
		...
		if (!gl) {
			console.error('[fluid] Unable to initialize WebGL.');
		...
		if (opts.hdr && 'drawingBufferColorSpace' in gl) {
			try {
			...
			if (import.meta.env.DEV) {
				console.info(
					renderLevel === 'webgl-p3'
						? '[fluid] HDR level: webgl-p3 (wide gamut fallback)'
						: '[fluid] HDR level: webgl-sdr (display-p3 drawing buffer rejected)'
				);
			}
		}
	...
	function updateCanvasRectCache() {
		if (opts.contained && canvas) canvasRectCache = canvas.getBoundingClientRect();
	}
	if (opts.contained) {
	...
	return {
		 ...
		 * yields yEngine = 1 - y — the same path a real mouse event's clientY
		 * takes. This is the only Y flip in the codebase.
		 */
